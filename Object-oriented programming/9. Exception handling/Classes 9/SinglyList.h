...
void SinglyList<T>::insert(int index, const T& data)
{
	...
	if (list_size == size_t{ 0 })
		{ pushFirstNode(new_node); ++list_size; return; }

	size_t normalize_index = normalizeIndex(index);
	if (normalize_index == size_t{ 0 })
		pushFrontNode(new_node);
	else {
...
void SinglyList<T>::remove(int index)
{
	...
	if (list_size == size_t{ 1 }) 
		{ popFirstNode(); delete remove_node; --list_size; return; }
...
template<typename T>
inline void SinglyList<T>::operator*(const double& multiplier)
{
	if (list_size == size_t{ 0 }) return;
	...
	{
		current_node->data = current_node->data * multiplier;
		current_node = current_node->pointer_to_next_node;
	}
}
